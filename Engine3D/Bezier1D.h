...
public:
	Bezier1D() : Bezier1D(3, 100, 3, 0) {}
	Bezier1D(int segNum, int res,int mode, int viewport = 0);
	IndexedModel GetLine(std::vector<glm::mat4> startingSegments);	//generates a model for MeshConstructor Constructor with resT vertices. See Mesh.h/cpp for a IndexedModel defenition and how to intialize it
	...
	static std::vector<glm::mat4> StartingSegments();
